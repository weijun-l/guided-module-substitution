...
def get_encoder(model, model_type):
...
def variance_accuracy_val(model, dataloader, device, max_len):
    validation_preds = []
    attention_variance = []
    all_targets = []
    ...
    with torch.no_grad():
        for index, batch in tqdm(enumerate(dataloader)):
            ...
            temp_matrix = compute_cls_variance(attention_matrix, attention_variance_layer, max_len=max_len)
...
def prune_heads_by_variance(pruned_model, model_type, dataloader, variance_matrix, prune_step, device, threshold=0.85):
    """
    ...
    for index, start_idx in enumerate(range(0, len(sorted_scores), prune_step)):
        ...
        get_encoder(model, model_type).prune_heads(pruned_heads_dict)   

        # Evaluate the pruned model on the validation set
        accuracy_val = evaluate_on_val(model, dataloader, device)
        back_track_count = 0
        ...
        while accuracy_val < threshold:
            ...
            accuracy_val = evaluate_on_val(model, dataloader, device)  # Renew the accuracy
...
def compute_norm_coefficient(model, dataloader, device, max_len):
    attention_variance = []
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader):
            ...
            attention_variance.append(compute_coefficient(attention_matrix, coefficient_list, max_len))
...
def save_coefficient_to_txt(coefficients, filename):
...
def main(args, model, tokenizer):
    ...
    # Load the Defender dataset [Use the clean data for pruning]
    full_train_dataset = load_json_dataset(args.train_clean)
    train_test_dict = full_train_dataset.train_test_split(train_size=0.9, seed=11)  # 90% for training, 10% for validation
    
    clean_train_df = train_test_dict['train'].to_pandas()
    clean_val_df = train_test_dict['test'].to_pandas()
    ...
    victim_model = get_model_wrapper(model).to(device)
    ...
    for epoch in tqdm(range(args.defending_epoch)):
        train_loss = train(victim_model, clean_train_dataloader, optimizer, loss_fn, scheduler, device)
    ...
    variance_score, accuracy = variance_accuracy_val(
        victim_model, 
        clean_val_dataloader,
        device,
        args.max_len_short
    )
    print(evaluate_on_val(victim_model, clean_val_dataloader, device))

    # Attention head pruning process [PURE Step 1: 3 - 9]
    Heads = prune_heads_by_variance(victim_model,
                                  model_type,  # New added.
                                  clean_val_dataloader,
    ...
    save_to_file(Heads, os.path.join(args.output_dir, "head_coefficients/pruned_heads.json"))
    ...
    get_encoder(model, model_type).prune_heads(Heads)

    print("Computing the norm coefficients...")
    norm_coefficients = compute_norm_coefficient(
        model, 
        clean_val_dataloader,
        device,
        args.max_len_short
    )
    save_coefficient_to_txt(norm_coefficients, os.path.join(args.output_dir, "head_coefficients/norm_coefficients.txt"))
